...
export function getSession() {
  ...
  // For development, provide a fallback session secret
  ...
  // Use in-memory store for development if DATABASE_URL is not available
  let sessionStore;
  if (process.env.DATABASE_URL) {
    try {
      ...
      console.warn('PostgreSQL session store failed, using memory store for development:', error);
      sessionStore = undefined; // Will default to memory store
    }
  } else {
